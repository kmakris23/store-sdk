...
describe('ProductAttributeService', () => {
  ...
  it('lists attributes with headers', async () => {
    mockedGet.mockResolvedValue({ data: [attr(1,'Color')], headers: { 'x-wp-total':1,'x-wp-totalpages':1 } });
    const result = await service.list();
    expect(mockedGet).toHaveBeenCalledWith('/wp-json/wc/store/v1/products/attributes');
  ...
  it('gets single attribute', async () => {
    ...
    expect(mockedGet).toHaveBeenCalledWith('/wp-json/wc/store/v1/products/attributes/5');
